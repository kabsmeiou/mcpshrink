...
def extract_tool_metadata(tools: List[FunctionTool]) -> List[dict]:
    tool_metadata = []
    for tool in tools:
...
async def get_mcp_tools(mcp_server) -> List[dict]:
    tools = await mcp_server.get_tools()
    tool_metadata = extract_tool_metadata(list(tools.values()))
    return tool_metadata


def generate_templates_for_all_tools(tool_metadata: List[dict]) -> List[dict]:
    records = []
    for tool in tool_metadata:
        templates = generate_template(tool_metadata=tool)
        records.extend(format_templates(templates, tool["name"]))
    return records


def expand_templates_for_all_records(records: List[dict]) -> List[dict]:
    expanded_records = []
